...
import { View, Text, Alert, TouchableOpacity, StyleSheet, TextInput, Keyboard, ScrollView, Image } from 'react-native';
...
export const MapScreen = () => {
    ...
    const fetchData = useCallback(async () => {
        try {
            // Fetch places
            const placesRes = await fetch(`${API_URL}/places?status=active`);
    ...
    const filteredPlaces = React.useMemo(() => (places || []).filter(place => {
    ...
    // Zoom to fit filtered places when category changes
    useEffect(() => {
        if (selectedCategory && filteredPlaces.length > 0) {
            setFollowUser(false);
    ...
    const handleSheetAnimate = (fromIndex: number, toIndex: number) => {
        // Hide buttons immediately when moving away from collapsed state (index 0)
        setIsSheetExpanded(toIndex > 0);
    };
...
const styles = StyleSheet.create({
    ...
    locationButton: {
        position: 'absolute',
        bottom: 260, // Moved up further to avoid overlap
        right: 24,
        width: 64, // Increased to match add button
        height: 64, // Increased to match add button
        backgroundColor: 'white',
    ...
    addButton: {
        position: 'absolute',
        bottom: 180, // Moved up further to avoid overlap
        right: 24,
        width: 64,
        height: 64,
        backgroundColor: '#0F766E',
